...
namespace FusionDirectorPlugin.Service
{
    ...
    public partial class FusionDirectorSyncInstance
    {
        ...
        #region sync current open alarms of FusionDirector
        /// <summary>
        /// Synchronizes open alarms from FusionDirector.
        /// </summary>
        public async void SyncFusionDirectorOpenAlarms()
        {
            // TODO(turnbig) should we unsubscribe before sync?
            logger.Polling.Info($"[SyncOpenAlarms] Start sync open alarms for FusionDirector `{FusionDirectorIp}`.");
            ...
            while (currentPage <= totalPages && IsRunning)
            {
                ...
                {
                    logger.Polling.Error(ex, $"[SyncOpenAlarms] Failed to sync open alarms from FusionDirector: {FusionDirectorIp}. Page number: {currentPage}.");
                }
                finally
                {
                    currentPage++;

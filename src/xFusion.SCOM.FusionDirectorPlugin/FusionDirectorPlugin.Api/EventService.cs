    ...
    public class EventService : BaseService
    {
        ...
        public async Task<string> GetCompanyName()
        {
            ...
            try
            {
                ...
                if (json["Oem"] != null)
                {
                    ...
                    foreach (JProperty jp in Oem.Properties())
                    {
                        if (jp.Name != null && jp.Name.Length > 0)
                        {
                            companyName = jp.Name;
                        }
                    }
        ...
        public async Task<Uri> CreateSubscriptionAsync(CreateSubscriptionBody body)
        {
            ...
            try
            {
                ...
                {
                    JObject Oem = (JObject)json["Oem"];
                    Oem.Add(companyName, Oem[defaultCompany]);
                    Oem.Remove(defaultCompany);
                }

                var content = new StringContent(JsonConvert.SerializeObject(json));
                ProcessRequest(url, json);
